...
Pyramid::Pyramid(Graphics& gfx, std::mt19937& rng, std::uniform_real_distribution<float>& adist,
                 ...
                 float width, float height, float depth, const FaceColors& colors)
    : r(rdist(rng))
    , droll(ddist(rng))
    , dpitch(ddist(rng))
    , dyaw(ddist(rng))
    ...
    , depthScale(depth)
{
    ...
    const ConstantBuffer2 cb2{ { colors[0], colors[1], colors[2], colors[3], colors[4] } };
...
{
    return DirectX::XMMatrixScaling(widthScale, heightScale, depthScale) *
